...
2. **An adult opens the door.** Claude Opus 4.8 reads the whole thing and
   walks in on it, opening with *"Vad fan håller ni på med?"* — baffled,
   exasperated, and the only participant here not bound to rhyme.
3. **The room laughs it off.** Each poet answers with one line, and the first
   of them rhymes with whatever the adult just said. The one voice that
   refuses to rhyme gets rhymed with anyway. Nobody apologises. Nothing stops.
4. On its way out the adult names the **mood** and describes the **scene** —
   the only part of the interruption the camera actually listens to.
5. **You get all of it once.** None of it is written to disk — gone the moment
   you close the terminal.
6. **The picture is taken.** Grok draws the three poets, four times over,
   each frame a moment apart.
7. If you handed in a real photo, the poets are keyed out of their flat
   background and composited into it. If you didn't, the whole scene is drawn.
8. The frames become one GIF in `photos/`. Everything else dies with the TTL.
...
The date, and how it felt. The mood comes from the adult, which is the only
